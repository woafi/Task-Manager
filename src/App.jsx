...
function App() {
  ...
  useEffect(() => {
    ...
    if (todoString) {
      setTodos(JSON.parse(todoString)) // Load saved todos
    }
  }, [])
    ...
    <>
      ...
      <div className="container bg-blue-100 rounded-xl my-7 mx-auto p-5 min-h-[80vh]">
        ...
        <div className="addTodo flex">
          <input type="text" value={todo} onChange={handleChange} placeholder='Task Title' className='bg-white w-1/2 rounded-md px-3' />
        ...
        <div className="display pl-5 ">
          ...
          {todos.map((item, index) => {
            return (showFinished || !item.isCompleted) && <div key={index} className='task-box  flex w-4/5 justify-between my-2 items-center'>
              
              <input type="checkbox" checked={item.isCompleted} onChange={() => handleCheckbox(index)} />
              {/* condition */}
              {editIndex === index ? (
                <input type="text" value={editText} onChange={(e) => setEditText(e.target.value)} className="bg-white w-2/3 px-2 rounded edit-tex" />
              ) : (
